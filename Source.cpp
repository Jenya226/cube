...
float fFar = 100.;
float fPov = 103.;
float fAspectRatio = (float)iScreenHeight / (float)iScreenWidth;
float fFovRad = (fPov / 180) * M_PI;
...
int main(int argc, char* argv[])
{
	...
	while (running) {
		...
		float fRotation_matZ[3][3] =
		{
			{cosf(fTheta),-sinf(fTheta),0},
			{sinf(fTheta), cosf(fTheta),0},
			{0,0,1}
		};
		float fRotation_matX[3][3] =
		{
			{1,0,0},
			{0, cosf(fTheta),-sinf(fTheta)},
			{0, sinf(fTheta),cosf(fTheta)}
		};

		float fRotation_matY[3][3] =
		{
			{cosf(fTheta),0,sinf(fTheta)},
			{0, 1,0},
		...
		Mult_tringl_by_matr(tPolygon_Mass, fRotation_matXZ); 
		Draw_Black_Screen();
		SDL_Delay(10);
